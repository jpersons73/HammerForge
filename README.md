...
```
The Mining Flow
Block Template: The stratum server calls `getblocktemplate` on your local full node to get a real block template containing pending transactions and the current block header data.
Job Construction: The server constructs a stratum mining job from the template, including the coinbase transaction (which pays the block reward to YOUR wallet address), the merkle branches, and the block header fields.
Work Distribution: Connected miners receive the job and begin hashing. Each miner gets a unique `extranonce1` so they never duplicate work.
Share Submission: When a miner finds a hash below the share difficulty target, it submits the share. The server validates the hash by reconstructing the full block header and performing a SHA-256d hash.
Block Found: If the share hash also meets the network difficulty target, the server immediately assembles the full block (header + coinbase + transactions) and submits it to your node via `submitblock`. A popup notification appears on your screen with the block details.
Difficulty System
...
Disk Space: Depends on how many coins you install and whether you enable pruning (see Blockchain Pruning Guide)
RAM: 8 GB minimum recommended if running multiple coin nodes simultaneously
Step 1: Download and Extract
Download the HammerForge ZIP package
Right-click the ZIP file and select Extract All...
Extract to any folder (e.g., your Desktop or `C:\HammerForge`)
Open the extracted folder - you should see `START-HERE.html` and the installer batch files
...
See the full Remote Access with Tailscale section below for detailed instructions.
...
Asks if you want to enable blockchain pruning (see Blockchain Pruning Guide)
...
Enable pruning? (y/n)
```
Type `y` to save disk space. Type `n` to keep the full blockchain. See the detailed Blockchain Pruning Guide for more information.
Step 5: Set Up Mining
...
4/7	Email notification setup (optional - see Email Notifications)
...
Step 6: Start Mining
Make sure your wallet is open and fully synced for the coin you want to mine
Double-click the desktop shortcut (e.g., "BC2 HammerForge")
A CMD window opens showing the server log and the monitoring dashboard opens in your browser
...
Password: x
```
Where `PORT` is the stratum port for the coin you want to mine (see Supported Coins table).
Examples
...
Go back to Security, scroll down to App passwords (or search for it)
Click App passwords
Select Mail as the app and Windows Computer as the device
Click Generate
Google shows a 16-character password (e.g., `abcd efgh ijkl mnop`)
Copy this password and paste it into the installer when prompted for "App password"
You only need to do this once - the password is saved in your config
...
Copy the generated password and use it during installation
Yahoo Setup
Go to login.yahoo.com/account/security
Click Generate app password
Select Other App and name it "HammerForge"
Copy the generated password and use it during installation
Per-Miner Email via Password Field
...
Sign in with your Google, Microsoft, or GitHub account (free for personal use)
Wait for Tailscale to connect - you will see the Tailscale icon in your system tray (bottom-right near the clock)
Verify it is working by opening a command prompt and running:
```
   tailscale ip -4
   ```
...
Mac/Linux: Download from tailscale.com/download
...
If found, displays your Tailscale IP (e.g., `100.64.0.2`)
Configures the batch files to detect and display the Tailscale URL when mining starts
The start scripts will show: `Remote access: http://100.64.0.2:5000/monitor`
...
Start mining any coin on your PC
Note the Tailscale URL shown in the CMD window (e.g., `http://100.64.0.2:5000/monitor`)
On your phone/other device, open a browser and go to that URL
You will see the full mining dashboard with live hashrate, workers, shares, and blocks
The connection is:
...
Re-run `INSTALL-MINING.bat` - it will detect Tailscale and update your batch files
Access manually - just find your Tailscale IP (`tailscale ip -4` in command prompt) and go to `http://YOUR_TAILSCALE_IP:5000/monitor`
---
BTC Fast Sync: UTXO Snapshots
...
GitHub: github.com/nicehash/bitcoin-utxo-snapshot
...
Close the wallet for that coin if it is running
...
To enable pruning, add or change the line:
```
   prune=10000
   ```
(Use the prune value from the table above for your coin)
To disable pruning, change the line to:
```
   prune=0
   ```
...
Start mining any coin - The stratum server automatically opens the port on your router via UPnP
Note your public IP - It is shown in the CMD window: `UPnP: External/public IP is X.X.X.X`
Create a new order on NiceHash:
Algorithm: SHA256
...
Popup window appears on your Windows desktop with full block details:
```
===================================
  BLOCK FOUND!
===================================
...
===================================
```
Email notification is sent (if configured) with the same details in a formatted HTML email
Log file entry is appended to `logs/blocks-found.log` (permanent record of all blocks found)
...
The server reconstructs the coinbase transaction using `coinb1 + extranonce1 + extranonce2 + coinb2`
Builds the merkle root by hashing the coinbase with the merkle branches
Constructs the 80-byte block header
Performs SHA-256d (double SHA-256) on the header
Compares the resulting hash against the share difficulty target
If the hash also meets the network target, submits the full block
Fallback Templates
